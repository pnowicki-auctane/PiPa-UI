    ...
    if not _HERMES_FOUND or not imports_ok:
        ...
        if provider == "custom" and not base_url:
            ...
            note = (
                ...
                "Configure the provider through PiPa settings or your provider's "
                "authentication flow, then reload. Advanced inherited configuration "
                "may still use the upstream 'hermes auth' or 'hermes model' commands."
            )
        else:
            note = (
                "Hermes has a saved provider/model selection but still needs the "
