    ...
    if _provider_is_oauth(provider_id):
        return {
            ...
            "error": f"'{_PROVIDER_DISPLAY.get(provider_id, provider_id)}' uses OAuth authentication. "
                     f"Configure it through PiPa settings or your provider's authentication flow. "
                     f"Advanced inherited configuration may still use the upstream `hermes model` command.",
        }
