...
def handle_get(handler, parsed) -> bool:
    ...
    if parsed.path == "/login":
        ...
        _raw_bn = _settings.get("bot_name") or ""
        _bn = _html.escape(_raw_bn if _raw_bn and _raw_bn != "Hermes" else "PiPa")
    ...
    # ── PiPa Skills API (GET) ──
    if parsed.path == "/api/pipa/skills":
        from api.pipa_skill_loader import iter_pipa_skills
        skills = iter_pipa_skills()
        return j(handler, {
            "ok": True,
            "skills": [
                {
...
def handle_post(handler, parsed) -> bool:
    ...
    if parsed.path == "/api/local-skill-run":
        return _handle_local_skill_run(handler, body)
    ...
    if parsed.path == "/api/settings":
        ...
        if "bot_name" in body:
            _candidate = (str(body["bot_name"]) or "").strip()
            # Migrate legacy "Hermes" value to "PiPa"; preserve real custom names.
            body["bot_name"] = _candidate if _candidate and _candidate != "Hermes" else "PiPa"
...
# ── PiPa Local Skill Runtime handler ─────────────────────────────────────────

def _handle_local_skill_run(handler, body):
    """POST /api/local-skill-run — run a selected PiPa skill against a local
    LM Studio endpoint with a clean, minimal prompt (no tool catalog, no
    all-skills context, no Hermes runtime instructions).
    """
    from api.pipa_skill_runtime import run_local_skill
    ...
    if not skill_id:
        return bad(handler, "skillId is required", 400)
    if not message:
        return bad(handler, "message is required", 400)
    if not model:
        return bad(handler, "model is required", 400)
    if not base_url:
        return bad(handler, "baseUrl is required", 400)
