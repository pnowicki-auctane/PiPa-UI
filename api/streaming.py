...
def _classify_provider_error(err_str: str, exc=None, *, silent_failure: bool = False) -> dict:
    ...
    if _is_quota:
        return {
            ...
            'hint': 'Your provider account is out of credits or usage. Top up, wait for the plan window to reset, or switch providers in PiPa settings.',
        }
    if _is_rate_limit:
        return {
    ...
    if _is_auth:
        return {
            ...
            'type': 'auth_mismatch',
            'hint': 'The selected model may not be supported by your configured provider or your API key is invalid. Check the provider/model in PiPa settings and verify your credentials.',
        }
    if _is_not_found:
        return {
            'label': 'Model not found',
            'type': 'model_not_found',
            'hint': 'The selected model was not found by the provider. Check the model ID in PiPa settings and verify it is available for your provider.',
        }
    ...
    if silent_failure:
        return {
            ...
            'type': 'no_response',
            'hint': 'The provider returned no content and no error. This often means a usage/rate limit was hit silently. Check provider status in PiPa settings or try again in a moment.',
        }
    ...
    try:
                    ...
                    if _is_quota:
                        ...
                        if not _assistant_added:
                            ...
                            _err_hint = (
                                ...
                                'your API key is invalid. Check the provider/model in PiPa settings '
        ...
        if _exc_is_quota:
            ...
            _exc_label, _exc_type, _exc_hint = (
                ...
                'Check the provider/model in PiPa settings.',
            )
