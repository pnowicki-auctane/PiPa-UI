#!/usr/bin/env python3
"""One-shot bootstrap launcher for PiPa."""
...
def install_hermes_agent() -> None:
    ...
    info(f"PiPa local agent not found. Attempting install via {INSTALLER_URL}")
    subprocess.run(
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Bootstrap PiPa onboarding.")
...
def main() -> int:
    ...
    if foreground_reason:
        info(
            f"Starting PiPa on http://{args.host}:{args.port} "
    ...
    info(f"Starting PiPa on http://{args.host}:{args.port}")
