...
function applyBotName(){
  ...
  const msg=$('msg');
  // Don't overwrite PiPa skill placeholder if a skill is already active.
  if(msg && !window._activePipaSkill) msg.placeholder='Message '+name+'\u2026';
}
...
(async()=>{
  ...
  try{
    ...
    window._botName=(!s.bot_name||s.bot_name==='Hermes')?'PiPa':s.bot_name;
  ...
  applyBotName();
  // If a PiPa skill was active before reload, restore the runtime chip
  // immediately so the Hermes model selector never flashes visible.
  ...
  const _hydrateBootModelDropdown=()=>populateModelDropdown({preferProfileDefaultOnFreshBoot:true}).then(()=>{
    ...
    // After model dropdown is fully populated, re-apply PiPa chip state so
    // the Hermes model chip is hidden if a PiPa skill is active.
    if (typeof syncPipaRuntimeChip === 'function') syncPipaRuntimeChip();
  }).catch(e=>{
...
async function shutdownServer() {
  const ok = await showConfirmDialog({
    title: (typeof t === 'function' ? t('settings_shutdown_confirm_title') : 'Stop PiPa'),
    message: (typeof t === 'function' ? t('settings_shutdown_confirm_message') : 'Stop the PiPa server?'),
