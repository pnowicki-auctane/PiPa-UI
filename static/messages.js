...
// Restore active skill from localStorage on load.
// Active PiPa skill is scoped to the current session (not global).
// Restored per-session in _pipaRestoreForCurrentSession() (panels.js) on every
// session switch and new-chat. Do NOT restore from a global key here.
window._activePipaSkill = null;
// One-time migration: remove the old global skill key so it cannot leak.
try { localStorage.removeItem('pipa-active-skill'); } catch(_) {}

// Persist/load PiPa model settings (these ARE global — model config is shared).
window._pipaSettings = (function(){
  try {
    const saved = localStorage.getItem('pipa-settings');
    if (saved) return JSON.parse(saved);
  } catch(_) {}
  return { model: 'qwen2.5-coder-14b-instruct-128k', baseUrl: 'http://localhost:1234/v1', temperature: 0.1, maxTokens: 2000 };
})();

// Helper: read active PiPa skill for the current session only.
// No localStorage fallback — skill is injected by _pipaRestoreForCurrentSession().
function _getActivePipaSkill() {
  if (window._activePipaSkill && window._activePipaSkill.id) return window._activePipaSkill;
  return null;
}

// ── PiPa pending indicator ──
let _pipaPendingTimer = null;
let _pipaPendingStart = 0;
const _PIPA_PENDING_ROW_ID = 'pipaPendingRow';

function _pipaShowPending(skillName) {
  _pipaPendingStart = Date.now();
  const inner = document.getElementById('msgInner');
  if (inner) {
    let row = document.getElementById(_PIPA_PENDING_ROW_ID);
    if (!row) {
      row = document.createElement('div');
      row.id = _PIPA_PENDING_ROW_ID;
      row.style.cssText = 'padding:10px 24px;color:var(--muted);font-size:13px;display:flex;align-items:center;gap:8px';
      row.innerHTML = '<span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:var(--accent);animation:pipaThinkPulse 1.2s ease-in-out infinite"></span>'
        + '<span id="pipaPendingLabel">PiPa is thinking…</span>';
      inner.appendChild(row);
      inner.scrollTop = inner.scrollHeight;
    }
  }
  if (!document.getElementById('pipaThinkStyle')) {
    const style = document.createElement('style');
    style.id = 'pipaThinkStyle';
    style.textContent = '@keyframes pipaThinkPulse{0%,100%{opacity:.3}50%{opacity:1}}';
    document.head.appendChild(style);
  }
  if (typeof setComposerStatus === 'function') setComposerStatus('PiPa is thinking…');
  _pipaPendingTimer = setInterval(() => {
    const elapsed = Math.round((Date.now() - _pipaPendingStart) / 1000);
    const label = document.getElementById('pipaPendingLabel');
    if (label) label.textContent = `PiPa is thinking… ${elapsed}s`;
    if (typeof setComposerStatus === 'function') setComposerStatus(`PiPa is thinking… ${elapsed}s`);
  }, 1000);
}

function _pipaHidePending() {
  if (_pipaPendingTimer) { clearInterval(_pipaPendingTimer); _pipaPendingTimer = null; }
  const row = document.getElementById(_PIPA_PENDING_ROW_ID);
  if (row) row.remove();
  if (typeof setComposerStatus === 'function') setComposerStatus('');
}

async function _pipaSend(text, skill) {
  console.log('[PiPa] Sending through local skill runtime. Skill:', skill.id);
  const settings = window._pipaSettings || {};
  const payload = {
    skillId: skill.id, message: text,
    model:       String(settings.model || 'qwen2.5-coder-14b-instruct-128k'),
    baseUrl:     String(settings.baseUrl || 'http://localhost:1234/v1'),
    temperature: Number(settings.temperature ?? 0.1),
    maxTokens:   Number(settings.maxTokens ?? 2000),
  };
  ...
  _pipaShowPending(skill.name);
  let result;
  const startMs = Date.now();
  try {
    result = await api('/api/local-skill-run', {method:'POST', body:JSON.stringify(payload)});
  } catch(e) {
    _pipaHidePending();
    S.messages.push({role:'assistant', content:`**PiPa runtime error**\n\nCould not reach the local backend at \`${payload.baseUrl}\`.\n\n> ${e.message}\n\nCheck that your OpenAI-compatible backend is running and the Base URL in Settings → PiPa is correct.`, _ts:Date.now()/1000});
    if(typeof renderMessages === 'function') renderMessages();
    console.error('[PiPa] /api/local-skill-run request failed:', e);
    return;
  }
  const elapsedMs = Date.now() - startMs;
  _pipaHidePending();
  if(result && result.ok) {
    const elapsed = elapsedMs > 999 ? ` · ${(elapsedMs/1000).toFixed(1)}s` : '';
    const content = result.response + '\n\n---\n*⚡ PiPa local · `' + skill.id + '` · model: `' + payload.model + '`' + elapsed + '*';
    S.messages.push({role:'assistant', content, _ts:Date.now()/1000});
    if(typeof renderMessages === 'function') renderMessages();
    console.log('[PiPa] Success in', elapsedMs + 'ms. Debug:', result.debug);
  } else {
    const errMsg = (result && result.error) || 'Unknown error from PiPa runtime';
    S.messages.push({role:'assistant', content:`**PiPa runtime error**\n\n> ${errMsg}`, _ts:Date.now()/1000});
    if(typeof renderMessages === 'function') renderMessages();
    console.warn('[PiPa] run failed:', result);
  }
}
// ─────────────────────────────────────────────────────────────────────────────

async function send(){
  // ── PiPa local skill runtime intercept ──────────────────────────────────
  // MUST be the absolute first check — before _sendInProgress, before session
  // creation, before any Hermes provider/runtime logic.
  {
    const _pipaSkill = _getActivePipaSkill();
    const _pipaText = ($('msg') && $('msg').value || '').trim();
    if (_pipaSkill && _pipaText) {
      if (_sendInProgress && S.busy) {
        if (typeof showToast === 'function') showToast('PiPa: please wait for current response to finish', 2000);
        return;
      }
      _sendInProgress = true;
      try { await _pipaSend(_pipaText, _pipaSkill); }
