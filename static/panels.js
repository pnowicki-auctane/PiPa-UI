...
function _cronGatewayNoticeHtml(status) {
  ...
  const body = notConfigured
    ? 'In PiPa, scheduled jobs require the gateway daemon. If this is a single-container Docker install, jobs can be created and run manually here, but scheduled ticks need a gateway container or `hermes gateway` running outside the WebUI.'
    : isStaleMetadata
      ...
      : isRemoteUnreachable
        ...
        : 'In PiPa, scheduled jobs require the gateway daemon to be running. Start the gateway container or `hermes gateway` before relying on offline scheduled runs.';
...
// ── PiPa skills ───────────────────────────────────────────────────────────────
let _pipaSkillsData = null; // cached PiPa skill list from /api/pipa/skills

// Sync the PiPa runtime chip in the composer footer.
function syncPipaRuntimeChip() {
  const chip = document.getElementById('pipaRuntimeChip');
  const label = document.getElementById('pipaRuntimeChipLabel');
  const modelChipWrap = document.querySelector('.composer-model-wrap');
  const skill = window._activePipaSkill;
  const settings = window._pipaSettings || {};
  if (skill && skill.id) {
    if (chip) chip.style.display = '';
    if (modelChipWrap) modelChipWrap.style.display = 'none';
    if (label) {
      const model = (settings.model || 'qwen2.5-coder-14b-instruct-128k').split('/').pop();
      const baseUrl = (settings.baseUrl || 'http://localhost:1234/v1').replace(/^https?:\/\//, '');
      label.textContent = `PiPa local · ${model} · ${baseUrl}`;
      if (chip) chip.title = `Runtime: PiPa local skill runtime\nModel: ${settings.model || 'qwen2.5-coder-14b-instruct-128k'}\nBase URL: ${settings.baseUrl || 'http://localhost:1234/v1'}\nTemperature: ${settings.temperature ?? 0.1}  Max tokens: ${settings.maxTokens ?? 2000}\n\nClick to edit PiPa settings`;
    }
  } else {
...
function _renderPipaSkillsSection(box, skills) {
  ...
  sec.style.cssText = 'border-top:1px solid var(--border2);margin-top:8px;padding-top:8px';
  const hdr = document.createElement('div');
  hdr.className = 'skills-cat-header';
  hdr.style.cssText = 'font-size:11px;font-weight:700;letter-spacing:.06em;color:var(--accent);padding:4px 0 6px 0;text-transform:uppercase';
  hdr.textContent = '⚡ PiPa PM Agent';
  sec.appendChild(hdr);
  for (const skill of skills) {
    ...
    const badge = document.createElement('span');
    badge.textContent = 'PiPa';
    badge.style.cssText = 'font-size:9px;font-weight:700;background:var(--accent);color:#fff;border-radius:3px;padding:1px 4px;margin-right:6px;letter-spacing:.04em;flex-shrink:0';
    ...
    el.append(badge, nameEl, descEl);
    el.dataset.pipaSkillId = skill.id;
    el.addEventListener('click', () => _selectPipaSkill(skill, el, sec));
    sec.appendChild(el);
  }
  box.appendChild(sec);
...
function _pipaSkillSessionKey() {
  return (S && S.session && S.session.session_id) ? S.session.session_id : null;
}

function _pipaSaveSkillForSession(skillObj) {
  const sid = _pipaSkillSessionKey();
  if (!sid) return;
  try {
    const map = JSON.parse(localStorage.getItem('pipa-active-skill-by-session') || '{}');
    if (skillObj) { map[sid] = skillObj; } else { delete map[sid]; }
    localStorage.setItem('pipa-active-skill-by-session', JSON.stringify(map));
  } catch(_) {}
}

function _pipaLoadSkillForSession(sid) {
  const key = sid || _pipaSkillSessionKey();
  if (!key) return null;
  try {
    const map = JSON.parse(localStorage.getItem('pipa-active-skill-by-session') || '{}');
    const entry = map[key];
    if (entry && entry.id && entry.name) return entry;
  } catch(_) {}
  return null;
}

function _selectPipaSkill(skill, el, sec) {
  document.querySelectorAll('.skill-item[data-pipa-skill-id]').forEach(e => {
    e.style.background = ''; e.style.outline = ''; e.style.outlineOffset = '';
  });
  if (window._activePipaSkill && window._activePipaSkill.id === skill.id) {
    window._activePipaSkill = null;
    _pipaSaveSkillForSession(null);
    const ta = document.getElementById('msg');
    if (ta) ta.placeholder = 'Message PiPa…';
    syncPipaRuntimeChip();
    console.log('[PiPa] Deselected skill');
    return;
  }
  const skillObj = { id: skill.id, name: skill.name };
  window._activePipaSkill = skillObj;
  _pipaSaveSkillForSession(skillObj);
  el.style.background = 'var(--accent-dim, rgba(99,102,241,0.15))';
  ...
  console.log('[PiPa] Selected skill:', skill.id, '| window._activePipaSkill =', window._activePipaSkill);
}

function _pipaRestoreSelectionHighlight() {
  if (!window._activePipaSkill) { syncPipaRuntimeChip(); return; }
  const activeId = window._activePipaSkill.id;
  document.querySelectorAll('.skill-item[data-pipa-skill-id]').forEach(e => {
    if (e.dataset.pipaSkillId === activeId) {
      e.style.background = 'var(--accent-dim, rgba(99,102,241,0.15))';
  ...
  const ta = document.getElementById('msg');
  if (ta) ta.placeholder = `PiPa — ${window._activePipaSkill.name}…`;
  syncPipaRuntimeChip();
}

function _clearActivePipaSkill() {
  window._activePipaSkill = null;
  _pipaSaveSkillForSession(null);
  document.querySelectorAll('.skill-item[data-pipa-skill-id]').forEach(e => {
    e.style.background = ''; e.style.outline = '';
  });
  const ta = document.getElementById('msg');
  if (ta) ta.placeholder = 'Message PiPa…';
  syncPipaRuntimeChip();
...
// Restore PiPa skill state for the session now loaded into S.session.
// Called on session switch and new session creation.
function _pipaRestoreForCurrentSession() {
  const skill = _pipaLoadSkillForSession();
  window._activePipaSkill = skill || null;
  const ta = document.getElementById('msg');
  if (skill) {
    if (ta) ta.placeholder = `PiPa — ${skill.name}…`;
  } else {
...
async function loadSkills() {
  ...
  try {
    const [hermesData, pipaData] = await Promise.all([
      api('/api/skills'),
      api('/api/pipa/skills').catch(() => ({skills:[]})),
    ]);
    _skillsData = hermesData.skills || [];
    _pipaSkillsData = pipaData.skills || [];
...
function switchSettingsSection(name){
  ...
  let section=(name==='appearance'||name==='preferences'||name==='providers'||name==='plugins'||name==='system'||name==='help'||name==='pipa')?name:'conversation';
  ...
  const map={conversation:'Conversation',appearance:'Appearance',preferences:'Preferences',providers:'Providers',plugins:'Plugins',system:'System',help:'Help',pipa:'Pipa'};
  // Sidebar menu items
  document.querySelectorAll('#settingsMenu .side-menu-item').forEach(it=>{
    it.classList.toggle('active', it.dataset.settingsSection===section);
  });
  // Panes in main
  ['conversation','appearance','preferences','providers','plugins','system','help','pipa'].forEach(key=>{
  ...
  if(section==='pipa') loadPipaSettingsPane();
}

// ── PiPa settings pane ────────────────────────────────────────────────────────
function loadPipaSettingsPane() {
  const settings = window._pipaSettings || {};
  const modelEl = $('pipaModel');
  const baseUrlEl = $('pipaBaseUrl');
  const tempEl = $('pipaTemperature');
  const maxTokEl = $('pipaMaxTokens');
  if (modelEl && !modelEl.value) modelEl.value = settings.model || 'qwen2.5-coder-14b-instruct-128k';
  if (baseUrlEl && !baseUrlEl.value) baseUrlEl.value = settings.baseUrl || 'http://localhost:1234/v1';
  if (tempEl && !tempEl.value) tempEl.value = settings.temperature != null ? settings.temperature : 0.1;
  if (maxTokEl && !maxTokEl.value) maxTokEl.value = settings.maxTokens != null ? settings.maxTokens : 2000;
}

function savePipaSettings() {
  const model = ($('pipaModel') || {}).value || 'qwen2.5-coder-14b-instruct-128k';
  const baseUrl = ($('pipaBaseUrl') || {}).value || 'http://localhost:1234/v1';
  const temperature = parseFloat(($('pipaTemperature') || {}).value) || 0.1;
  const maxTokens = parseInt(($('pipaMaxTokens') || {}).value, 10) || 2000;
  window._pipaSettings = { model, baseUrl, temperature, maxTokens };
  try { localStorage.setItem('pipa-settings', JSON.stringify(window._pipaSettings)); } catch(_) {}
  const status = $('pipaSaveStatus');
...
function _buildProviderCard(p){
  ...
  if(isOauth){
    ...
    if(p.key_source==='config_yaml'){
      hint.textContent=t('providers_oauth_config_yaml_hint')||'Token configured via config.yaml. To update, edit the providers section in your config.yaml. Advanced inherited configuration may still use the upstream hermes auth command.';
      ...
      hint.textContent=t('providers_oauth_not_configured_hint')||'Not authenticated. Configure this provider through PiPa settings. Advanced inherited configuration may still use the upstream hermes auth command.';
  ...
  if(p.configurable){
    ...
    hint.textContent=p.is_custom
      ? 'Custom provider loaded from config.yaml. Edit the providers section in config.yaml to update it.'
...
function _applySavedSettingsUi(saved, body, opts){
  ...
  // PiPa migration: treat legacy persisted "Hermes" as "PiPa".
  window._botName=(!body.bot_name||body.bot_name==='Hermes')?'PiPa':body.bot_name;
  if(typeof applyBotName==='function') applyBotName();
