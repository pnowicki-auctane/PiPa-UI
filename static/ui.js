...
async function populateModelDropdown(opts={}){
  ...
  try{
    ...
    if(!groups.length) {
      // No provider is configured — replace whatever static options are showing
      // with an explicit placeholder so the chip never displays a fake cloud model.
      sel.innerHTML = '<option value="" data-no-provider="1">No provider configured</option>';
      if(typeof syncModelChip==='function') syncModelChip();
      return;
    }
...
function _checkProviderMismatch(modelId){
  ...
  if(norm(modelProvider)!==norm(ap)){
    return (window.t?window.t('provider_mismatch_warning',modelId,ap):
      `"${modelId}" may not work with your configured provider (${ap}). Send anyway, or change the provider/model in PiPa settings. Advanced inherited configuration may still use the upstream \`hermes model\` command.`);
  }
  return null;
}
...
function syncModelChip(){
  ...
  const noProvider=!!(opt&&opt.dataset&&opt.dataset.noProvider)||(!sel.value&&sel.options.length===1&&sel.options[0].dataset.noProvider);
  if(noProvider){
    label.textContent='No provider configured';
    if(mobileLabel) mobileLabel.textContent='No provider configured';
    chip.title='No chat provider configured — configure a provider in Settings, or use PiPa local skills';
...
function _showAgentHealthAlert(payload){
  ...
  if(!banner) return;
  if(title) title.textContent='PiPa agent is not responding';
